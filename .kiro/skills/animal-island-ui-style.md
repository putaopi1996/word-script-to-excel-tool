...
**底部状态图标：**
```css
.iconWifi     { width: 79px; height: 29px; }
.iconLocation { width: 36px; height: 36px; }
.iconPage     { width: 65px; height: 32px; }
...
.tree         { height: 60px; background-size: cover; background-position: bottom center; }
```
...
| keyword  | `#d4a0e0` | `import/export/const/return/async/...` |
| react    | `#e06c75` | `React/useState/useEffect/FC/ReactNode/...` |
| component| `#80c0e0` | 大写驼峰标识符 |
| func     | `#61afef` | 小写标识符后跟 `(` |
| prop     | `#e8c87a` | 标识符后跟 `=` |
| jsx      | `#f0a870` | `<Tag`、`</Tag`、`/>` |
| operator | `#d4b896` | `{}[]();,` 等 |
...
每个组件在 `demo/components/<ComponentName>/index.tsx` 创建演示页，并在 `demo/ComponentPage.tsx` 中注册路由，同时把 `title / desc` 写入 `demo/pageInfo.ts`。
